...
static void ide_write_register(uint8_t channel, uint8_t reg, uint8_t data) {
    if (reg > 0x07 && reg < 0x0C)
        ide_write_register(
            channel,
            ATA_REG_CONTROL,
            0x80 | g_ide_channels[channel].no_intr
        );
    if (reg < 0x08)
        writeb_port(g_ide_channels[channel].base + reg - 0x00, data);
    else if (reg < 0x0C)
        writeb_port(g_ide_channels[channel].base + reg - 0x06, data);
    else if (reg < 0x0E)
        writeb_port(g_ide_channels[channel].control + reg - 0x0A, data);
    else if (reg < 0x16)
        writeb_port(g_ide_channels[channel].bm_ide + reg - 0x0E, data);
    if (reg > 0x07 && reg < 0x0C)
        ide_write_register(
            channel,
            ATA_REG_CONTROL,
            g_ide_channels[channel].no_intr
        );
}
...
static uint8_t ide_read_register(uint8_t channel, uint8_t reg) {
    ...
    if (reg < 0x08)
        ret = readb_port(g_ide_channels[channel].base + reg - 0x00);
    else if (reg < 0x0C)
        ret = readb_port(g_ide_channels[channel].base + reg - 0x06);
    else if (reg < 0x0E)
        ret = readb_port(g_ide_channels[channel].control + reg - 0x0A);
    else if (reg < 0x16)
        ret = readb_port(g_ide_channels[channel].bm_ide + reg - 0x0E);
    if (reg > 0x07 && reg < 0x0C)
        ide_write_register(
            channel,
            ATA_REG_CONTROL,
...
uint8_t ide_print_error(uint32_t drive, uint8_t err) {
    ...
    p_printf("IDE:");
    if (err == 1) {
        p_printf("- Device Fault\n");
        err = 19;
    } else if (err == 2) {
        uint8_t st = ide_read_register(g_ide_devices[drive].channel, ATA_REG_ERROR);
        if (st & ATA_ER_AMNF) {
            p_printf("- No Address Mark Found\n");
            err = 7;
        }
        if (st & ATA_ER_TK0NF) {
            p_printf("- No Media or Media Error\n");
            err = 3;
        }
        if (st & ATA_ER_ABRT) {
            p_printf("- Command Aborted\n");
            err = 20;
        }
        if (st & ATA_ER_MCR) {
            p_printf("- No Media or Media Error\n");
        ...
        if (st & ATA_ER_UNC) {
            p_printf("- Uncorrectable Data Error\n");
            err = 22;
        }
        if (st & ATA_ER_BBK) {
            p_printf("- Bad Sectors\n");
            err = 13;
        }
    } else if (err == 3) {
        p_printf("- Reads Nothing\n");
        err = 23;
    } else if (err == 4) {
        p_printf("- Write Protected\n");
        err = 8;
    }
    p_printf("- [%s %s] %s\n",
...
void init_ide(uint32_t prim_channel_base_addr, uint32_t prim_channel_control_base_addr,
...
int ide_read_sectors(uint8_t drive, uint8_t num_sectors, uint32_t lba, uint32_t buffer) {
    if (drive > MAXIMUM_IDE_DEVICES || g_ide_devices[drive].reserved == 0) {
        p_printf("IDE ERROR: Drive not found\n");
        return -1;
    }
    else if (((lba + num_sectors) > g_ide_devices[drive].size) && (g_ide_devices[drive].type == IDE_ATA)) {
        p_printf("IDE ERROR: LBA address(0x%x) is greater than the available drive sectors(0x%x)\n", lba, g_ide_devices[drive].size);
        return -2;
    }
    else {
        uint8_t err;
...
int get_ata_drive_by_model(const char *model) {
    int i;
    for(i = 0; i < MAXIMUM_IDE_DEVICES; i++) {
        if(s_strcmp((char *)g_ide_devices[i].model, (char *)model) == 0)
            return i;
    }
    return -1;
}
